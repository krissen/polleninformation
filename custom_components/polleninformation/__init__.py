...
from .const import (CONF_COUNTRY, CONF_COUNTRY_ID, CONF_LANG, CONF_LANG_ID,
                    CONF_LATITUDE, CONF_LONGITUDE, DEFAULT_COUNTRY,
                    DEFAULT_LANG, DEFAULT_LANG_ID, DEFAULT_LATITUDE,
                    DEFAULT_LONGITUDE, DOMAIN, PLATFORMS)
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if DEBUG:
        _LOGGER.debug("INIT: Setup entry with lat=%s, lon=%s, country=%s, country_id=%s, lang=%s, lang_id=%s",
                      lat, lon, country, country_id, lang, lang_id)
...
class PollenInformationDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> dict:
        if DEBUG:
            _LOGGER.debug("COORDINATOR: Update data with lat=%s, lon=%s, country=%s, country_id=%s, lang=%s, lang_id=%s",
                          self.lat, self.lon, self.country, self.country_id, self.lang, self.lang_id)
        try:
            result = await async_get_pollenat_data(
                self.hass, self.lat, self.lon, self.country, self.country_id, self.lang, self.lang_id
            )
