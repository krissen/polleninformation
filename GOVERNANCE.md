...
Polleninformation EU is maintained by Kristian Niemi
...
`master`. A change is merged when the maintainer has reviewed it and CI passes:
lint and formatting, secret scanning, the test suite, and HACS and hassfest
validation.
...
pull request, the release is tagged, and a GitHub release is published with
release notes. Larger changes go out as a pre-release (beta) first.
