...
BEMÖDA DIG OM ATT BEHÅLLA DET ICKE RENDERADE HELA RETURNERINGEN IGENOM! Jag vet att det är svårt, i synnerhet mellan block. Men du fixar det! Rå icke-renderad markdown hela vägen nu!
...
1. Laddar in befintlig `country_ids.json` (om filen inte finns, skapas en tom struktur).  
2. Kollar om landet (t.ex. "SE") redan har en sparad match i höger nod; om ja → hoppar över.  
3. Annars loopar skriptet genom:  
...
4. När ett giltigt anrop hittas (finns `"contamination"` i svaret) första gången:  
   - Markera `country_id` som “matchad” och spara in i JSON.  
   - Avbryt vidare test för detta land.
...
- JSON‐filen `country_ids.json` uppdateras med t.ex.  
...
- Lagrar tre nyckelnoder i JSON:  
  1. `"countries"`: kartläggning av landskod → (lista `country_ids`, `lat`, `lon`, `place_slug`, `place_format` osv).  
  2. `"tested"`: per‐land lista över alla `country_id`‐värden som just detta skript provat.  
  3. `"invalid"`: lista över `country_id` som verkligen gav ogiltigt/null‐svar (får aldrig matchas senare, men kan köras på nytt i undantagsfall).

**När du använder detta:**

1. Kör om du vill testa hela Europa (du ger in en lista av flertal EU-länder i koden).  
2. För varje land i din `EUROPEAN_LOCATIONS`-lista:  
...
1. Laddar `country_ids.json`.  
2. För varje land i `"countries"`, hämtar `info["place_format"]` (t.ex. ”8001 Zürich”).  
3. Genererar en ny slug med den uppdaterade `slugify(...)`-funktionen.  
4. Om den nya slugen skiljer sig från befintlig `place_slug` i JSON → skriva över + uppdatera `"last_updated"`.  
5. I slutändan sparas en temporär `.tmp`-fil och byts ut mot originalet atomärt, så att du inte riskerar halvskriven fil.
...
1. Laddar in `country_ids.json`.  
2. För varje `{ country: info }` i `"countries"`:  
   a. Hämta befintligt lat/lon från JSON (detta är koordinaten vi använde för att testa).  
   b. Hämta det sparade `place_format` (t.ex. “LV-4242 Ipiķi”).  
   c. Försök att geokoda med `hint = "<place_format>, <country>"` via geopy.  
   d. Om lyckad: få `(lat2, lon2)` samt bekräftat land.  
   e. Beräkna avstånd i km mellan `(lat, lon)` (ursprunglig sökpunkt) och `(lat2, lon2)`.  
   f. Hämta landkod via en enkel “reverse‐lookup” på `lat2/lon2`.  
   g. Om `landkod == förväntat` → markera som **valid**. Annars → markera som **invalid** med skäl (t.ex. “felaktig landmatchning (förväntat=VA, geokodat=IT)”).  
   h. Om “hint” ger ingen träff → försök utan hint (`"<place_format>"`).  
   i. Om fortfarande ingen träff → sätt `landkod = None` → markera för manuell kontroll i JSON.

3. I slutändan skapas (eller skrivs över) en ny version av `country_ids.json` där varje land‐nod får en extra under‐nod `"validation"` med fältet:
...
1. Läser in “final” `country_ids.json`.  
2. Använder interna Python‐moduler (t.ex. `pycountry` eller ett eget uppslagsverk) för att programmässigt hämta landet namn (“Sweden”, “Italy” osv) baserat på landskoden (“SE”, “IT”).  
3. Skapar en ny JSON‐fil, t.ex. `available_countries.json` med struktur:
...
4. Sparar filen under `custom_components/polleninformation/` (eller annat lämpligt ställe) så att integrationens Python-kod enklare kan ladda den utan att behöva kontakta API:t i realtid.
...
- Alternativt kan skriptet direkt spara → `custom_components/polleninformation/available_countries.json`.  
...
1. Fyll på `country_ids.json` med giltiga `country_id`  
...
2. Validera alla “tertiära” träffar  
...
3. (Om vi ändrat slug-logiken) Migrera befintliga `place_slug`  
...
4. Generera ett “tillgängliga länder”-utdrag som integrationen kan läsa  
...
Kör de olika skripten i ordning enligt avsnittet “Arbetsflöde”. Varje steg uppdaterar `country_ids.json` så att nästa steg vet vad som redan är testat, ogiltigt eller matchat.
...
- **`test_pollenapi_countryid.py` / `test_pollenapi_multi.py`:** Loopa igenom `country_id`‐värden för att hitta och spara vilket ID som verkligen fungerar för varje land.  
